...
function start() {
  ...
  // row 1 is for entry lists, row 2 is for daily minute counts
  myArray = [[], []];
  ...
  for (let i = data.indexOf(fve); i < data.length; i++) {
    ...
    if (e.title == "The Disastrous Life of Saiki K.") e.episodes = e.watchedepisodes = 24
  ...
  for (let j = 0; j < dayzero + offset1 + 1; j++) {
    ...
    if (val != 0 && !isNaN(val)) {
      curbox.setAttribute(
        "onclick",
        "localStorage.setItem('specialTransfer','" +
          JSON.stringify(myArray[0][i]) +
          "');window.open('GroupDetails.html','_blank').focus();"
      );
    }
    if (i < 0) {
